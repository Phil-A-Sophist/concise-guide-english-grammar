# ENGL 3110 — Bonus Assignment — Spring 2026

**Total Points: 20  ·  2 points per sentence**

---

## Instructions

For each sentence below, complete two tasks:

1. **Complete the labeling table** by filling in the Role, Phrase, and Part of Speech (POS) rows.
2. **Create a syntax tree diagram** and paste your diagram below.

**Scoring (per sentence):** 1 point for a correct labeling table  ·  1 point for a correct diagram

---

## Abbreviation Key

| Parts of Speech    | Phrase Types               | Roles            |
|--------------------|---------------------------|------------------|
| Noun = N           | Noun Phrase = NP           | Subject = Subj   |
| Verb = V           | Verb Phrase = VP           | Predicate = Pred |
| Adjective = ADJ    | Adjective Phrase = ADJP    |                  |
| Adverb = ADV       | Adverb Phrase = ADVP       |                  |
| Pronoun = PRO      | Prepositional Phrase = PP  |                  |
| Determiner = DET   | Sentence = S               |                  |
| Preposition = PREP |                            |                  |

**Notes:**
- In the Phrase row, label the main verb as VP.
- In the Role row, label the subject NP as Subject and the main verb as Predicate. Leave all other cells empty.
- Always include a Phrase-level label for every noun, verb, adjective, adverb, and preposition.
- Determiners (DET) group under the NP — they do not get their own Phrase label.
- When a PP follows a noun inside an NP, label the NP for the head noun (and any pre-nominal adjectives), then label the PP separately.
- When a PP or ADJP follows a noun as a post-nominal modifier, label it separately from the NP.

---

## Sentences

**Sentence 1:** The tall energetic student near the door arrived late.

| Role   |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |
| Word   | The | tall | energetic | student | near | the | door | arrived | late |
| POS    |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 2:** The brilliant young professor from the northern campus spoke clearly.

| Role   |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |
| Word   | The | brilliant | young | professor | from | the | northern | campus | spoke | clearly |
| POS    |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 3:** The student afraid of failure studied very diligently.

| Role   |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |
| Word   | The | student | afraid | of | failure | studied | very | diligently |
| POS    |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 4:** They lived quite far from the noisy old city.

| Role   |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |
| Word   | They | lived | quite | far | from | the | noisy | old | city |
| POS    |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 5:** He waited quietly at the bench near the old fountain.

| Role   |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |
| Word   | He | waited | quietly | at | the | bench | near | the | old | fountain |
| POS    |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 6:** She sat quite peacefully near the tall fountain in the old courtyard.

| Role   |  |  |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |  |  |
| Word   | She | sat | quite | peacefully | near | the | tall | fountain | in | the | old | courtyard |
| POS    |  |  |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 7:** The tired old dog from the farm slept quietly near the fire.

| Role   |  |  |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |  |  |
| Word   | The | tired | old | dog | from | the | farm | slept | quietly | near | the | fire |
| POS    |  |  |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 8:** The nervous boy sat rigidly in the hard narrow chair.

| Role   |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |
| Word   | The | nervous | boy | sat | rigidly | in | the | hard | narrow | chair |
| POS    |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 9:** The small birds sang very loudly deep in the dark forest.

| Role   |  |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |  |
| Word   | The | small | birds | sang | very | loudly | deep | in | the | dark | forest |
| POS    |  |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]

---

**Sentence 10:** The old woman confident in her skills spoke quite passionately.

| Role   |  |  |  |  |  |  |  |  |  |
|--------|--|--|--|--|--|--|--|--|--|
| Phrase |  |  |  |  |  |  |  |  |  |
| Word   | The | old | woman | confident | in | her | skills | spoke | quite | passionately |
| POS    |  |  |  |  |  |  |  |  |  |  |

Bracket notation:

Diagram: [paste your diagram here]
