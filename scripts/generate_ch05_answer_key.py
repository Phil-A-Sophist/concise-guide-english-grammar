...
def create_answer_key(output_path, font_size=12, overhead=False):
    ...
    run = p.add_run('a) Which sentence feels clearest or most direct to you? Why?')
    run.bold = True
    run.font.size = Pt(body_size)
    run.font.name = body_font
    ...
    p = doc.add_paragraph()
    p.paragraph_format.left_indent = Inches(0.35)
    run = p.add_run('b) What does each version emphasize?')
    run.bold = True
    run.font.size = Pt(body_size)
    run.font.name = body_font
    set_paragraph_spacing(p, space_before=3, space_after=2)
    ...
    p = doc.add_paragraph()
    p.paragraph_format.left_indent = Inches(0.35)
    run = p.add_run('c) In what writing situation might you choose one version over another?')
    run.bold = True
    run.font.size = Pt(body_size)
    run.font.name = body_font
    set_paragraph_spacing(p, space_before=3, space_after=2)
