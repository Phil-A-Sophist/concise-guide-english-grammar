...
def create_answer_key(output_path, font_size=12, overhead=False):
    ...
    for i, (num, sentence, clause, classification) in enumerate(complements):
        if i > 0:
            question_page_break(doc, overhead)
    ...
    for i, (num, sentence, ctype, effect) in enumerate(constructions):
        if i > 0:
            question_page_break(doc, overhead)
        add_exercise(doc, num, sentence, body_size, font_name=body_font)
        answer_page_break(doc, overhead)
    ...
    answer_page_break(doc, overhead)
    add_plain_line(doc,
        '20A) Simple sentence: The lack of evidence surprised the investigators.',
        body_size, font_name=body_font)
    add_plain_line(doc,
        '20B) It-cleft: It was the lack of evidence that surprised the investigators.',
        body_size, font_name=body_font)
